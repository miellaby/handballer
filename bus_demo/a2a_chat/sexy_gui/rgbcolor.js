...
RGBColor.prototype = {
    ...
    derived: function (ratio, wb) {
        var c = new RGBColor(this);
        if (wb) { // lighter
            c.r = 255 - (255 - c.r) * ratio;
            c.g = 255 - (255 - c.g) * ratio;
            c.b = 255 - (255 - c.b) * ratio;
        } else { // darker
            c.r *= ratio;
            c.g *= ratio;
            c.b *= ratio;
...
(function () {
    ...
    RGBColor.darker = function (color) {
        if (!cache[color])
            cache[color] = new RGBColor(color).derived(0.7);
        return cache[color];
    };
    var cachel = {};
    RGBColor.lighter = function (color) {
        if (!cachel[color])
            cachel[color] = new RGBColor(color).derived(0.7, true);
        return cachel[color];
    };
})();
