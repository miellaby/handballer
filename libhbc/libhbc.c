...
void *thread_loop(void *thing) {
  ...
  for (;;) {
    /* wait a message */
    if (hbc_wait(socketTabEntry->socket, &label, &body) < 0)
      break;
    /* call the callback */
...
static int hbc_init() {
  ...
  if ((env = getenv("HANDBALLER")) != NULL) {
    int port, nmatch = sscanf(env,
                              "%" PARAM_STRING_MAX_LENGTH_F
                              "[^:]:%d%" PARAM_STRING_MAX_LENGTH_F "s",
                              hbc_dynamic_hostname, &port, hbc_dynamic_prefix);
    if (nmatch < 1) {
  ...
  bcopy((char *)_hbcServer->h_addr, (char *)&_hbcAddr.sin_addr.s_addr,
        _hbcServer->h_length);
...
int hbc_get(char *pattern, hbc_callback_t callback, void *context) {
  ...
  if (connect(hbcGetSocket, (struct sockaddr *)&_hbcAddr,
  ...
  if (pthread_create(&pthread_id, NULL, thread_loop, (void *)&socketTab[i]) !=
      0) {
...
unsigned long hbc_bodySize(int hbcGetSocket) {
  int i;
  for (i = 0; i < 5; i++)
    if (socketTab[i].pid && socketTab[i].socket == hbcGetSocket)
      break;
...
int _hbc_wait(int hbcGetSocket, char **label_p, char **body_p, int headerOnly) {
  ...
  if (entry->body_offset > 0) { // last message removing
    if (entry->read_offset > entry->body_offset)
      memcpy(entry->read_buffer, entry->read_buffer + entry->body_offset,
  ...
  while (1) {
    ...
    do {
      ...
      if (!entry->labelSize_offset) {
        ...
        if (!sep)
          continue;
        *sep = '\0';
        sscanf(entry->read_buffer + entry->header_offset, "%x",
               &entry->label_size);
        DBG("%d bytes in label", entry->label_size);
        entry->labelSize_offset =
            entry->header_offset +
            strlen(entry->read_buffer + entry->header_offset) + 1;
      }

      if (!entry->label_offset) {
        if (entry->read_offset - entry->labelSize_offset <
            entry->label_size + 1)
          continue;
      ...
      if (!entry->size_offset) {
        ...
        if (!sep)
          continue;
        *sep = '\0';
        sscanf(entry->read_buffer + entry->label_offset, "%lx",
               &entry->body_size);
        DBG("%ld bytes in body", entry->body_size);
        entry->size_offset = entry->label_offset +
                             strlen(entry->read_buffer + entry->label_offset) +
                             1;
      }
    ...
    n = recv(entry->socket, entry->read_buffer + entry->read_offset,
             entry->read_buffer_size - entry->read_offset - 1, 0);
    DBG("%d bytes read.", n);
    ...
    if (n + entry->read_offset >
        (entry->read_buffer_size / 2)) { // large memory margin requiered

      char *tmp =
          (char *)realloc(entry->read_buffer, entry->read_buffer_size * 2);
...
int hbc_poll(int hbcGetSocket, struct timeval *timeval_p) {
  ...
  if (!entry->header_read) { // we first jump over the HTTP headers of a fresh
                             // GET connection (blocking)
    _hbc_wait(hbcGetSocket, NULL, NULL, 1);
  }

  if (entry->read_offset > entry->header_offset &&
      entry->read_offset > entry->body_offset + 2)
...
int hbc_post(char *label, char *body) {
  ...
  if (connect(hbcPostSocket, (struct sockaddr *)&_hbcAddr,
              sizeof(struct sockaddr_in)) < 0) {
